...
| 已标注 | 4,785 |
| 待人工标注 | 4（网络下载实拍图） |
| 处方类型 | 19种 + 物理形变变体（透视/褶皱/遮挡/光照/泛黄） |
| 覆盖科室 | 65+ |
| 难度分布 | Easy: 269 / Medium: 527 / Hard: 708 |
...
│   ├── tcm/                     # 中药处方图片 (182张)
│   ├── western/                 # 西药处方图片 (181张)
│   ├── handwritten/             # 手写风格处方 (111张)
│   ├── mixed/                   # 中西医结合处方 (50张)
│   ├── inpatient/               # 住院处方 (60张)
│   ├── emergency/               # 急诊处方 (40张)
│   ├── granule/                 # 中药配方颗粒 (40张)
│   ├── special/                 # 特殊处方 (70张)
│   ├── patent_tcm/              # 中成药处方 (60张)
│   ├── topical/                 # 外用处方 (50张)
│   ├── complex/                 # 复杂多药处方 (60张)
│   ├── pediatric/               # 儿科处方 (50张)
│   ├── landscape/               # 横版处方 (50张)
│   ├── e_prescription/          # 电子处方 (160张)
│   ├── insurance/               # 医保处方 (160张)
│   ├── correction/              # 涂改处方 (105张)
│   ├── lab_prescription/        # 检验处方 (135张)
│   ├── variants/                # 变体处方 (2225张)
│   ├── variants2/               # 变体处方2 (280张)
│   ├── deformation/             # 物理形变处方 (800张)
│   └── correction_v2/           # 涂改处方v2 (200张)
├── scripts/
...
### 中成药处方（10 科室）
中医内科、心病科、脾胃科、肺病科、全科、骨伤科、妇科、皮肤科、脑病科、肾病科

### 外用处方（6 科室）
皮肤科、骨科、中医外科、眼科、耳鼻喉科、口腔科

### 复杂多药处方（5 科室）
心内科、内分泌科、肿瘤内科、风湿免疫科、呼吸内科

### 儿科处方（5 科室）
儿内科、新生儿科、儿童呼吸科、儿童皮肤科、儿童消化科

### 横版处方（4 科室）
神经内科、消化内科、心内科、呼吸内科

### 电子处方（4 科室）
内科、心内科、皮肤科、消化内科、中医内科
...
### 检验处方（5 科室）
内科、心内科、消化内科、内分泌科、肾内科

### 难度等级
- **Easy**（~800张）：白色背景，标准字体，清晰排版
- **Medium**（~1500张）：轻微噪点，模拟打印质量波动
- **Hard**（~2500张）：明显噪点，模拟实拍条件（纸张泛黄、字迹模糊、手写风格、旋转、印章、低分辨率、运动模糊、涂改痕迹、透视变换、褶皱、遮挡、光照不均）
